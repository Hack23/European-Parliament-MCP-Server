...
export async function withTimeout(promise, timeoutMs, errorMessage) {
    ...
    const timeoutPromise = new Promise((_, reject) => {
        timeoutHandle = setTimeout(() => {
            reject(new TimeoutError(errorMessage ?? `Operation timed out after ${String(timeoutMs)}ms`, timeoutMs));
        }, timeoutMs);
    });
 ...
 * ```
 */
export async function withTimeoutAndAbort(fn, timeoutMs, errorMessage) {
 ...
 */
function validateRetryOptions(maxRetries, timeoutMs, retryDelayMs) {
    if (maxRetries < 0) {
...
export async function withRetry(fn, options) {
    const { maxRetries, timeoutMs, retryDelayMs = 1000, timeoutErrorMessage, shouldRetry = () => true } = options;
    // Input validation
