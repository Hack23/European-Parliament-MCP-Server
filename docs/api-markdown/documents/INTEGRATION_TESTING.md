...
[European Parliament MCP Server API](../modules.md) / INTEGRATION\_TESTING
...
The 15 OSINT tools share a common metadata envelope ([`OsintStandardOutput`](../tools/shared/types/README.md) — `confidenceLevel`, `methodology`, `dataFreshness`, `sourceAttribution`, `dataQualityWarnings`). A single, registry-driven contract test enforces that envelope uniformly across every OSINT tool so regressions cannot land undetected.

**Location:** [`tests/integration/osint/contract.test.ts`](../_media/contract.test.ts)
...
1. **Envelope schema** — response payload parses against [`OsintStandardOutputSchema`](../schemas/ep/analysis/README.md).
