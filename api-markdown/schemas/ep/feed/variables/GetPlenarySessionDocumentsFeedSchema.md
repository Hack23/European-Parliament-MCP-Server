[**European Parliament MCP Server API v1.2.8**](../../../../README.md)

***

[European Parliament MCP Server API](../../../../modules.md) / [schemas/ep/feed](../README.md) / GetPlenarySessionDocumentsFeedSchema

# Variable: GetPlenarySessionDocumentsFeedSchema

> `const` **GetPlenarySessionDocumentsFeedSchema**: `ZodObject`\<\{ \}, `$strict`\> = `FixedWindowFeedSchema`

Defined in: [schemas/ep/feed.ts:78](https://github.com/Hack23/European-Parliament-MCP-Server/blob/main/src/schemas/ep/feed.ts#L78)

GET /plenary-session-documents/feed — server-default window
