...
| **46 MCP Tools** | `get_meps`, `get_mep_details`, `get_plenary_sessions`, `assess_mep_influence`, `analyze_country_delegation`, etc. | Core functionality — must degrade gracefully per tool |
...
| **Node.js Runtime** (≥ 25.x) | Runtime | 🟠 High | Pinned versions; current supported line (25.x Current, upgrading to 26.x LTS in ≈ April 2026). Starting with Node.js 27, new annual release model with every release becoming LTS (36-month support). See [End-of-Life-Strategy.md](End-of-Life-Strategy.md) |
...
| **Affected Tools** | All 47 MCP tools (`get_meps`, `get_plenary_sessions`, `assess_mep_influence`, `analyze_country_delegation`, etc.) |
...
| 6 | ✅ Once recovered: verify all 47 MCP tools return valid data | Maintainer | 10 min |
...
| **EP API Degradation Test** | All 47 MCP tools return structured error responses when EP API is mocked as unavailable |
